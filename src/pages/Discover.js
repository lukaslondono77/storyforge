  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {["All", "free", "paid"].map(t => (
            <button key={t} onClick={() => setTier(t)}
              className="btn btn-sm"
              style={{
                background: tier === t ? "var(--gold)" : "#fff",
                color: tier === t ? "#fff" : "var(--muted)",
                borderColor: tier === t ? "var(--gold)" : "var(--border)",
                fontSize: 12,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
        {filtered.map(story => <StoryCard key={story.slug} story={story} />)}
      </div>
    </div>
  );
}
      ...
      >
        ...
        <div style={{ padding: "22px 24px", display: "flex", flexDirection: "column", height: "calc(100% - 4px)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--border)", paddingTop: 14 }}>
            <span style={{ fontSize: 13, color: "var(--muted)" }}>by {story.author}</span>
            <div style={{ display: "flex", gap: 12 }}>
              <StatPill val={story.reads || 0} label="reads" />
              <StatPill val={story.subscribers || 0} label="subs" />
            </div>
          </div>
        </div>
      </div>
...
function StatPill({ val, label }) {
  ...
  return (
    <span style={{ fontSize: 12, color: "var(--dim)" }}>
      <span style={{ color: "var(--ink2)", fontWeight: 500 }}>{display}</span> {label}
    </span>
  );
}
